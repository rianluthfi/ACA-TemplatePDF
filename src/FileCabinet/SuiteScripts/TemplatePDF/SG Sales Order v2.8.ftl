...
<#--
    ...
    History:
	v2.8 - 2026-02-19 - Add condition to show item format for QUANTUM item and add tag p to maintain text-align left (Req by Felicia)
    v2.7 - 2026-02-11 - Update Bank Instruction to DBS Bank
-->
...
<pdf>
				...
				<table
					...
					<tr>
						<td style="padding-top: 2px; text-align: center; border-width: 0.6px;"
							align="center">
							<span style="font-size: 10px;">${record.memo}</span>
						</td>
						<td style="padding-top: 2px; text-align: center; border-width: 0.6px;"
							align="center">
	...
	<body header="nlheader" header-height="27%" footer="nlfooter" footer-height="13%"
		padding="0.5in 0.5in 0.5in 0.5in" size="Letter">
		
		<#if record.item?has_content>
			<table
				class="itemtable" style="width: 100%; margin-top: 10px;" border="1"><!-- start items -->
				<#list record.item as item>
					<#if item_index==0>
						<thead>
							<tr>
								...
								<th class="borderkanan borderbawah" align="center" width="5%">Qty<br />B/O</th>
                    ...
                    <#assign qtyOrder = (item.quantity?has_content)?then(item.quantity?replace(",", "")?number, 0)>
                    <#assign qtyInvoice = (item.quantitybilled?has_content)?then(item.quantitybilled?replace(",", "")?number, 0)>
                    <#assign qtyRemaining = qtyOrder - qtyInvoice>

                    <#if record.custbody_aca_only_show_incomplete == true>
                        <#if qtyRemaining gt 0>
                            <tr>
